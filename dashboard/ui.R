...
ui <- fluidPage("", theme = shinytheme("superhero"),
                ...
                tabsetPanel(type = c("pills"),
                  tabPanel("Einführung", icon = icon("play"), 
                           fluidRow(
                             column(12,
                                    br(),
                                    verticalTabsetPanel(
                                      verticalTabPanel("Heuristik", style = "color:#000;",
                                      ...
                                      verticalTabPanel("Simualted Annealing", style = "color:#000;",
                  ...
                  tabPanel("Daten", style = "background-color:#ffff;", icon = icon("table"),
                           DT::dataTableOutput("randomDataTable")),
                  tabPanel("Funktionsweise", icon = icon("robot"),
                           tabsetPanel(type = c("pills"),
                                       tabPanel("Optimum", icon = icon("signal-slash"),
                                                fluidRow(
                                                  column(12, h1("Lokale und globale Optima"),
                                                  br()
                                                )
                                                ),
                                       ),
                                       tabPanel("Dummy Code", icon = icon("code"),
                                                fluidRow(
                                                  column(12, h1("Funktionsweise des Algorithmus"),
                                                         br(),
                                                         img(src = "algorithm.png")
                                                  )
                                                ),
                                       ),
                                       tabPanel("Beispiel", icon = icon("simplybuilt"),
                                                fluidRow(
                  ...
                  tabPanel("Praxisbeispiel", icon = icon("route"),
